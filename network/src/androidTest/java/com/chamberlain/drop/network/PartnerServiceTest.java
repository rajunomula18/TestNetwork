...
public class PartnerServiceTest {
    ...
    @Test
    public void makeCallToGetPartnerList_verifyResponseSuccess() {
        CountDownLatch countDownLatch = new CountDownLatch(1);
        Map<String, String> params = new HashMap<>();
        params.put("brandId", "brandId");
        mockedNetworkFramework.partnerService().getPartnerList(params, new INetworkFramework.RawNetworkCallback() {
            @Override
    ...
    @Test
    public void makeCallToLinkPartner_verifyResponseSuccess() {
        CountDownLatch countDownLatch = new CountDownLatch(1);
        mockedNetworkFramework.partnerService().linkPartner("partnerId", new INetworkFramework.RawNetworkCallback() {
            @Override
            public void onResponse(NetworkResponse response) {
                assertTrue(response.isSuccessful());
                countDownLatch.countDown();
    ...
    @Test
    public void makeCallToUnlinkPartner_verifyResponseSuccess() {
        CountDownLatch countDownLatch = new CountDownLatch(1);
        mockedNetworkFramework.partnerService().unlinkPartner("partnerId", new INetworkFramework.RawNetworkCallback() {
            @Override
            public void onResponse(NetworkResponse response) {
                assertTrue(response.isSuccessful());
                countDownLatch.countDown();
